...
- Work Unit 5 wires validated stage selection into single-player: router-owned `stage_id`, bounded Stage selector, stage-specific arena/spawn/presentation runtime application, dynamic arena bounds for movement/Teleport/Grab/Summon, and Chromium/Edge regression for selection/fail-closed/persistence/passive-Training isolation. After CI #518 caught a GDScript constant-expression defect and commit `3fdc15f871b11c1b3bdb3431a597fd88020008bd` corrected it without semantic changes, PR #185 CI #520 passed Windows Native, Godot/domain/backend, Chromium and hosted Edge with `stageSelection=true`, `stageCount=2`, and `SMOKE_SUITE_PASSED count=25`; final docs-sync CI remains before merge.
- Work Unit 6 remains the final deployed single-player acceptance across selectable stage content.
