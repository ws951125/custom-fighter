...
V2-7 Work Unit 3 is implemented and PR-validated on `feat/v2-7-wu3-http-catalog`; PR #208 CI #607 (`36225607371`) passed all required gates and the work unit now awaits latest-head docs-sync validation plus explicit merge approval. The HTTP contract adds bounded publish, browse/search, publication/revision detail and exact immutable package download while production writes remain fail-closed without durable persistence and server-side publisher identity. After WU3 merge/validation, proceed to **Work Unit 4 — durable free-tier persistence + publisher identity** using no-cost infrastructure only.
