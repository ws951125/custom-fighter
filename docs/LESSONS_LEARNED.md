...
- **Validation:** PR #180 latest-head CI #497 (`35580807539`) passed Windows Native Release, Godot import/boot/domain/backend, Web export/size budget, Chromium `smoke:all`, and GitHub-hosted Windows Microsoft Edge `smoke:all` on head `f90a8928fe58e4d5bb35f45279b345d48a890f72`. The previously failing Buff cast-release observation and packaged-VFX `U`/MP acknowledgement path both completed without product/runtime changes. The same operational rule was revalidated on 2026-09-23 twice: PR #192 CI #547 attempt 1 timed out in unchanged `creator_package_vfx_web_smoke.mjs:252` and targeted same-SHA Edge retry passed the complete suite in attempt 2; PR #195 CI #561 attempt 1 timed out in unchanged `match_restart_web_smoke.mjs:100` after Chromium and all domain/product gates passed, while targeted same-SHA retry of only the failed Edge job passed the complete Edge suite in attempt 2 with no product/test change.
- **Status:** Verified on PR #180 CI #497; revalidated by PR #192 CI #547 attempt 2 and PR #195 CI #561 attempt 2
