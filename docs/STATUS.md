...
Work Unit 4 — **server adapter/schema implementation merged; production provider provisioning and live write/read acceptance still blocked**:
...
- Review hardening: new `sb_secret_` keys are now `apikey`-only (not Bearer JWT); project URL/key format is bounded, SQL validates stored UTF-8 size and SHA-256, caps aggregate package bytes at 128 MiB with a transaction lock, uses literal search semantics, and maps storage exhaustion to HTTP 507. Added regression and setup/lesson documentation. No real database schema was applied.
- Added deterministic backend regressions for the Supabase adapters and server package validator. Added `tests/supabase_schema_contract_test.mjs` so CI also freezes RLS, no-browser table grants, `SECURITY INVOKER`, service-role-only RPC execution, atomic revision locking and the 16 MiB storage bound. PR #210 latest head `4497ee7c22e5546dd450d49198c61e5068fc25ed` passed CI #626 (`36228811804`): Windows Native, Godot/backend/Web/Chromium and hosted Edge all reached SUCCESS.
- PR #210 was explicitly approved and squash-merged to `main` as `a154e748662d66305938fb81202234a03c4ffea5`. Render auto-deploy `dep-daroec7f3r2c73a8tgkg` reached `live` on that exact revision; backend startup was observed in Render logs at 2026-09-26T08:50:14Z.
- Main-push GitHub Actions runs are not exposed by the available GitHub connector (its commit-run endpoint lists PR-triggered runs only), and public URL retrieval was inaccessible from the web tool. Therefore exact-main Pages/public reachability and Production Edge full smoke remain unverified evidence, not PASS.
- Production provisioning remains blocked pending explicit authorization for a dedicated no-cost Supabase project and its server-side credentials. SQL was not applied, Render Supabase values were not provisioned, and the live catalog remains fail-closed without config. No existing unrelated Supabase project was modified or reused automatically.
- Next non-blocked slice: WU5 Creator Gallery UX against the WU3 HTTP contract, with graceful unavailable states until durable production provider authorization and cross-user acceptance.
...
- WU4 documentation PR #211 CI #628 succeeded and was explicitly approved/squash-merged into main as `1044e341231d2cdda9c7b00dad28f90310f10388`. WU4 implementation PR #210 was merged as `a154e748662d66305938fb81202234a03c4ffea5` and its exact revision was live on Render.
- First PR CI #629 (`36232444457`) found a Godot import parser failure in new `gallery_creator_studio.gd:331`: `var safe_cursor := next_cursor.length() <= 32` tried to infer a type from a Variant. Fixed on the same branch with an explicit `bool` and bounded string conversion; CI #638 revalidated this fix in both Chromium and hosted Edge.
- Hardened Godot JSON numeric boundary: revision and byte-size JSON numbers are accepted only when exact positive safe integers within their respective bounds (including float representation of whole numbers); fractional, string and out-of-range values are rejected before network download/import. This was identified during review and CI #638 completed SUCCESS on the hardened branch.
- Latest-head CI #634 (`36232747483`) completed FAILURE exclusively in the new Chromium `smoke:creator-gallery`: timeout after catalog browse at test line 119 waiting for publication selection/detail state. Godot import/domain/backend, Windows native, Web export, and earlier Chromium smokes completed successfully. Windows Edge was skipped due to the Chromium failure. The diagnostic Web callback had erroneously accepted only float-index arguments across JavaScriptBridge, rejecting integer index 0; the correction now handles exact nonnegative int/float indices, and the smoke prints UI/request telemetry on repeat failure. Latest-head CI #638 (`36234225075`) on `d560192cd59320c29b5add24db6cfaee44270750` completed SUCCESS, including Windows Native, Godot/backend/Web/Chromium and hosted Microsoft Edge. Both browsers logged `CREATOR_GALLERY_BROWSER_SMOKE_PASSED` and `SMOKE_SUITE_PASSED count=29`. No production or external provider mutation.
- Reconciliation commit `6320229e25fe600458a79c524432b22b138422d0` merged the PR #211 checkpoint into the #212 branch without merging #212 to main. New exact-head CI #640 (`36236310673`) completed SUCCESS: Windows Native, Godot/domain/backend/Web/Chromium and hosted Edge all succeeded; both browsers emitted `CREATOR_GALLERY_BROWSER_SMOKE_PASSED` and `SMOKE_SUITE_PASSED count=29`. The separate PR #211 exact-main CI #639 (`36236067345`) completed SUCCESS across all seven jobs, including GitHub Pages, public reachability, Production Backend Readiness, and Production Edge full smoke (28 stages); Render deployment `dep-darpu53ncjis73dsnj20` is live on the exact main SHA `1044e341231d2cdda9c7b00dad28f90310f10388`.
- Preview delivery gate: GitHub Pages public production does not contain PR #212 and its public PR-preview deployment feature is not generally available. There is no preexisting Custom Fighter standalone preview project in the available connected Vercel/Render services. A separate online preview site would require explicit new-service authorization, remain on a strictly zero-cost plan with no charges accepted, and leave the current GitHub Pages and Render backend unchanged. No new external site or paid resource has been created; an Actions artifact is build evidence, not an interactive online preview. Gallery manual gameplay/visual/UX acceptance is pending.
- Latest exact-head CI #641 (`36237326137`) on `27958deef62aeae54b5c3398b1e48f051aa4e46e` completed SUCCESS: Windows Native, Godot/backend/Web/Chromium and hosted Edge; both browsers emitted `CREATOR_GALLERY_BROWSER_SMOKE_PASSED` and `SMOKE_SUITE_PASSED count=29`.
- WU5 safe visual-review evidence slice: extend the existing Gallery Chromium smoke with five screenshots of mock-provider unavailable, catalogue list, publication detail, valid import and tampered-package rejection; publish short-retention PR-only Actions artifacts, including one directly viewable standalone PNG through `upload-artifact@v7` `archive:false`. This does not deploy an interactive preview or require a new hosted account/service. Mock data is explicitly test-only, not the unavailable production catalogue. CI #642 (`36240010652`) completed SUCCESS, including Chromium and Edge; six mock-provider screenshots were uploaded as short-retention GitHub artifacts.
- WU5 confirmation regression slice: the diagnostic Web callback previously bypassed the visible Gallery ConfirmationDialog. It now opens the exact same cancellable dialog as the actual button. Hosted browser smoke verifies no revision/package request or draft mutation before acceptance, Escape cancellation, then valid and tampered import through the UI accept key. PR-only mock-provider visual screenshots include a sixth confirmation-dialog image. CI #642 (`36240010652`) completed SUCCESS in Chromium and hosted Edge, covering dialog cancellation and accepted import. Screenshots and browser mocks are not a deployed preview or live provider.
- WU5 bounded Gallery browse review: cap each displayed API page at 20 records and total visible results at 100; block a repeated continuation cursor or an empty page with a non-empty cursor before appending, without mutating the Creator draft. Diagnostic Refresh/Next callbacks call the same metadata-only UI handlers. GitHub Chromium/Edge smoke now exercises oversized/non-advancing pages, 100-record cap and the original confirmed import flow. CI #644 (`36250217352`) completed SUCCESS on attempt 2 with both browsers logging `paginationBounded=true` and 29 smoke stages. An unchanged hosted Edge Network PvP timeout in attempt 1 passed on the same-SHA failed-job retry without product/test changes. No live provider or preview created.
- WU5 immutable revision binding hardening (2026-09-27): exact-revision metadata must preserve the selected publication's package and publisher IDs; after SHA-256/byte/UTF-8 checks, the downloaded JSON package envelope must also match manifest package ID/version/schema before invoking the existing Creator validator. Added mock-API Chromium/Edge regression for cross-package manifest substitution and a matching-digest but wrong-version manifest, checking no premature download/draft mutation. Exact-source-head CI #646 (`36256148248`) completed SUCCESS on attempt 2: Windows Native and Chromium passed first attempt; Edge first attempt failed in unchanged `network_pvp_online_acceptance_smoke.mjs:100` basic-attack ready wait after audio worklet network error, and its failed-only same-SHA retry passed with `NETWORK_PVP_WU6_ONLINE_ACCEPTANCE_PASSED`, `CREATOR_GALLERY_BROWSER_SMOKE_PASSED manifestEnvelopeBound=true`, and `SMOKE_SUITE_PASSED count=29`. No product/test/timeout change, external provider or deployment change.
- PR #212 remains OPEN and unmerged. This docs-only synchronization follows validated #646 implementation SHA and needs its own latest-head CI status. Publish/Auth, durable production catalogue, live interactive preview and cross-user acceptance remain incomplete; V2 stays 75% (6/8).
