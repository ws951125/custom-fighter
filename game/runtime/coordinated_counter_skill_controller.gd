extends "res://game/runtime/counter_skill_controller.gd"

const SKILL_OWNER := &"skill_11"
...
func _load_skill() -> void:
	...
	if character == null or not character.has_method("skill_id_for_slot"):
		return
	var requested_id := str(character.call("skill_id_for_slot", "skill_11"))
	if requested_id.is_empty():
		return
	var errors: PackedStringArray = host.load_character_skill_for_slot("skill_11", "counter", skill)
	if not errors.is_empty():
		push_error("Failed to load counter skill from character slot: %s" % " | ".join(errors))
		return
	cast_state.configure(skill)

func _process(delta: float) -> void:
	super(delta)
	if host != null and not cast_state.is_casting() and not counter_state.active:
