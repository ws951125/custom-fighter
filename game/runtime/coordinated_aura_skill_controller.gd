extends "res://game/runtime/aura_skill_controller.gd"

const SKILL_OWNER := &"skill_9"
...
func _load_skill() -> void:
	...
	if character == null or not character.has_method("skill_id_for_slot"):
		return
	var requested_id := str(character.call("skill_id_for_slot", "skill_9"))
	if requested_id.is_empty():
		return
	var errors: PackedStringArray = host.load_character_skill_for_slot("skill_9", "aura", skill)
	if not errors.is_empty():
		push_error("Failed to load aura skill from character slot: %s" % " | ".join(errors))
		return
