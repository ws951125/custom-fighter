extends "res://game/runtime/grab_skill_controller.gd"

const SKILL_OWNER := &"skill_12"
...
func _load_skill() -> void:
	...
	if character == null or not character.has_method("skill_id_for_slot"):
		return
	var requested_id := str(character.call("skill_id_for_slot", "skill_12"))
	if requested_id.is_empty():
		return
	var errors: PackedStringArray = host.load_character_skill_for_slot("skill_12", "grab", skill)
	if not errors.is_empty():
		push_error("Failed to load grab skill from character slot: %s" % " | ".join(errors))
		return
	cast_state.configure(skill)

func _process(delta: float) -> void:
	super(delta)
	if host != null and not cast_state.is_casting() and not grab_state.active:
...
func _try_cast() -> void:
	if not host.skill_coordinator.try_claim(SKILL_OWNER):
		return
	super()
	if not cast_state.is_casting() and not grab_state.active:
		host.skill_coordinator.release(SKILL_OWNER)
