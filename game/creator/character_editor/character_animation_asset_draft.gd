...
func configure_import(
	...
	animation_id_value: String,
	name: String,
	mime: String,
	width: int,
	height: int,
	frames: int = 1,
	fps_value: float = 12.0
) -> PackedStringArray:
	schema_version = CURRENT_SCHEMA_VERSION
	semantic = semantic_value.strip_edges().to_lower()
	animation_id = animation_id_value.strip_edges().to_lower()
...
func validate() -> PackedStringArray:
	...
	if not CharacterAnimationMap.REQUIRED_SEMANTICS.has(semantic):
		errors.append("animation asset semantic must use a required character animation semantic")
	if not _is_safe_token(animation_id):
		errors.append("animation asset animation_id must be a safe lowercase token")
	if not _is_safe_png_filename(file_name):
	...
	if frame_count > 0 and image_width > 0 and image_width % frame_count != 0:
...
func validate_bytes(png_bytes: PackedByteArray) -> PackedStringArray:
	...
	if image.get_width() != image_width or image.get_height() != image_height:
		errors.append("decoded animation asset PNG dimensions do not match metadata")
	return errors
...
func frame_height() -> int:
	return image_height if image_height > 0 else 0

func _is_safe_png_filename(value: String) -> bool:
	var normalized := value.strip_edges()
	if normalized.is_empty() or not normalized.to_lower().ends_with(".png"):
		return false
	if normalized.contains("/") or normalized.contains("\\") or normalized.contains(":") or normalized.contains(".."):
		return false
	var regex := RegEx.new()
	regex.compile("^[A-Za-z0-9][A-Za-z0-9._ -]*\\.png$")
