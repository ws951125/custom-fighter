...
var _stored_animation_asset_bytes := PackedByteArray()
...
var _preview_animation_asset_bytes := PackedByteArray()
...
func stage_preview(character_data: Dictionary, skill_data: Dictionary, animation_map_data: Dictionary = {}, audio_bindings_data: Dictionary = {}) -> PackedStringArray:
	...
	if not audio_bindings_data.is_empty() and has_stored_audio_asset():
		var audio_asset_errors: PackedStringArray = _validate_audio_asset_payload(
			_stored_audio_asset_data,
			_stored_audio_asset_bytes,
			audio_bindings_data
		)
		for error in audio_asset_errors:
			errors.append("character audio asset: %s" % error)
		if not errors.is_empty():
			_deactivate_failed_preview()
			return errors
	...
	if animation_map_data.is_empty():
		...
		if has_stored_animation_asset():
			...
			_preview_animation_asset_bytes = _stored_animation_asset_bytes.duplicate()
		else:
			_clear_preview_animation_asset()
	...
	if not audio_bindings_data.is_empty() and has_stored_audio_asset():
		_preview_audio_asset_data = _stored_audio_asset_data.duplicate(true)
		_preview_audio_asset_bytes = _stored_audio_asset_bytes.duplicate()
	else:
...
func store_animation_asset_draft(animation_asset_data: Dictionary, png_bytes: PackedByteArray) -> PackedStringArray:
	var errors := _validate_animation_asset_payload(animation_asset_data, png_bytes)
	if not errors.is_empty():
		clear_animation_asset_draft()
		return errors
	var draft := CharacterAnimationAssetDraft.new()
	var metadata_errors: PackedStringArray = draft.load_from_dictionary(animation_asset_data)
	if not metadata_errors.is_empty():
		clear_animation_asset_draft()
		return metadata_errors
	_stored_animation_asset_data = draft.to_dictionary()
	_stored_animation_asset_bytes = png_bytes.duplicate()
	_clear_preview_animation_asset()
	revision += 1
	return PackedStringArray()
...
func clear_animation_asset_draft() -> void:
	...
	_stored_animation_asset_bytes.clear()
	_clear_preview_animation_asset()
	revision += 1
...
func _validate_animation_asset_payload(
	animation_asset_data: Dictionary,
	png_bytes: PackedByteArray,
	animation_map_data: Dictionary = {}
) -> PackedStringArray:
	var errors := PackedStringArray()
	var draft := CharacterAnimationAssetDraft.new()
	var metadata_errors: PackedStringArray = draft.load_from_dictionary(animation_asset_data)
	...
	if metadata_errors.is_empty():
		...
		for error in byte_errors:
			errors.append(error)
	if not errors.is_empty() or animation_map_data.is_empty():
		return errors
	...
	if not errors.is_empty():
		return errors
	var mapped_animation := str(animation_map.animations.get(draft.semantic, "")).strip_edges().to_lower()
	if mapped_animation != draft.animation_id:
		errors.append("animation asset animation_id must match the active semantic mapping")
	return errors
...
func _validate_audio_asset_payload(audio_asset_data: Dictionary, wav_bytes: PackedByteArray, audio_bindings_data: Dictionary) -> PackedStringArray:
	var errors := PackedStringArray()
	var draft := CharacterAudioAssetDraft.new()
	var metadata_errors: PackedStringArray = draft.load_from_dictionary(audio_asset_data)
	for error in metadata_errors:
	...
	var bindings := CharacterAudioBindings.new()
	var binding_errors: PackedStringArray = bindings.load_from_dictionary(audio_bindings_data)
	for error in binding_errors:
		errors.append("audio bindings: %s" % error)
	if not errors.is_empty():
		return errors
	if bindings.cue_for_binding(draft.binding) != draft.cue_id:
		errors.append("WAV cue_id must match the active audio binding")
	return errors
...
func has_stored_animation_asset() -> bool:
	return not _stored_animation_asset_data.is_empty() and not _stored_animation_asset_bytes.is_empty()

func has_active_animation_asset_preview() -> bool:
	return has_active_preview() and not _preview_animation_asset_data.is_empty() and not _preview_animation_asset_bytes.is_empty()
...
func has_active_audio_asset_preview() -> bool:
	return has_active_preview() and not _preview_audio_asset_data.is_empty() and not _preview_audio_asset_bytes.is_empty()
...
func preview_animation_asset_bytes() -> PackedByteArray:
	return _preview_animation_asset_bytes.duplicate()

func preview_audio_bindings_data() -> Dictionary:
	return _preview_audio_bindings_data.duplicate(true)

func preview_audio_asset_data() -> Dictionary:
	return _preview_audio_asset_data.duplicate(true)

func preview_audio_asset_bytes() -> PackedByteArray:
	return _preview_audio_asset_bytes.duplicate()
...
func stored_animation_asset_bytes() -> PackedByteArray:
	return _stored_animation_asset_bytes.duplicate()
...
func clear() -> void:
	...
	_preview_animation_asset_bytes.clear()
	_preview_audio_bindings_data.clear()
