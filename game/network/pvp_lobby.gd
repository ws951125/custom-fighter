...
func _process(_delta: float) -> void:
	...
	elif ready_state == WebSocketPeer.STATE_CLOSED and _connection_state != "CLOSED":
...
func _build_ui() -> void:
	...
	var note := Label.new()
	note.text = "WU4 browser transport: lobby + validated loadout + ready/start + authoritative state"
...
func _refresh_ui() -> void:
	if _connection_label == null:
		return
	_connection_label.text = "Connection: %s · client=%s" % [_connection_state, _client_id]
	_lobby_label.text = "Lobby: %s · participants=%s" % [_lobby_id, JSON.stringify(_participants)]
	_match_label.text = "Match: %s · status=%s · tick=%d · players=%s" % [
		_match_id,
		_match_status,
		_server_tick,
